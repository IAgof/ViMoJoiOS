...
class EditorInteractor: NSObject, EditorInteractorInterface {
    ...
    func getListData(completion: ([EditorViewModel]) -> Void) {
        ...
        let videoList = self.videosList.map {
            EditorViewModel(
                thumbnail: $0.thumbnailImage ?? #imageLiteral(resourceName: "video_removed"),
                timeText: self.hourToString($0.getStopTime() - $0.getStartTime()),
                positionText: "\($0.getPosition())")
        }
